...
years_forward = projection_year - current_year
...
for name, ticker in stocks.items():
    ...
    results.append({
        ...
        "Projected Dividend 2026": float(projected_div),
        "Projected Yield 2026": float(projected_yield)
    })
...
df = df.sort_values("Projected Yield 2026", ascending=False)
...
for _, row in df.iterrows():
    ...
    print(f"  Projected Dividend {projection_year}: {row['Projected Dividend 2026']:.2f}")
    print(f"  Projected Yield {projection_year}: {row['Projected Yield 2026']:.2%}")
    print("")

print("===========================================================\n")

print("Income Ranking (Highest Yield First):\n")

for i, row in enumerate(df["Company"], 1):
    print(f"{i}. {row}")
