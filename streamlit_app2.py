...
def load_prices():
    ...
    if not check_connectivity():
        st.error("❌ No connection to Yahoo Finance API.")
        st.info("Check internet / DNS / firewall / SSL certificates.")
        return pd.DataFrame()
    ...
    if not frames:
        st.error("❌ All ticker downloads failed.")
        st.info("Possible causes:")
        st.write("- Firewall / proxy blocking Yahoo")
        st.write("- Missing SSL certificates")
        st.write("- Temporary Yahoo throttling")
        return pd.DataFrame()
